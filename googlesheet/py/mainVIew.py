...
class upDateSN(QtCore.QThread):
    ...
    def run(self):
        while (True):
            ...
            devicesn = ['dwdwdwqd', 'ddfsdf', 'WIP1234567890', 'WIP1234234333', 'WIP324325245245']
            # allsn = []
            self._connectSignal.emit(devicesn)
            self.sleep(1)
...
class view(QtWidgets.QWidget, Ui_Form):
    ...
    def backCar(self):
        if (len(self.SerialNumber.text()) != 13):
            ...
            info = str(self.queryData(self.SerialNumber.text()))
            ...
            if (allsn.__contains__(self.SerialNumber.text()) & len(info) > 800):
                ...
                WO_Number = info[info.find("WO=") + 3:info.find("WO=") + 12]
                ...
                if (not station_Status.__contains__(SN_Status)):
                    msg = QMessageBox.information(self, "提示：", "站别错误", QMessageBox.Yes)
                    self.SerialNumber.clear()
                    self.setEnabled(True)
                    return
                # 检查MBSN
                DUT_MB = os.popen('adb  shell cat /factory_setting/mlb_sn.txt').readlines()[0].replace('\n', '')
                if (DUT_MB != Mlb_SN):
                ...
                msg = QMessageBox.information(self, "提示：", "SN错误或SN不存在", QMessageBox.Yes)
                return
    ...
    def FAT(self):
        ...
        #     检查touch_detect
        testStr = os.popen('adb shell /home/flex/bin/fct.sh touch_detect').read()
        ...
        testItemc += 1
        self.template_1.setItem(testItemc, 8, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        test_mlb = os.popen('adb shell /home/flex/bin/fct.sh component_check_mlb').read()
        ...
        SEL_bad_blocks = os.popen('adb shell /home/flex/bin/fct.sh bad_blocks').read()
        self.RTestInfo.setText('\nNand_check:{0}'.format(SEL_bad_blocks))
        # Nand Flash Scan
        testItemc += 1
        self.template_1.setItem(testItemc, 8, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        test_nand = os.popen('adb shell /home/flex/bin/fct.sh nand_check 10').read()
        ...
        testItemc += 1
        self.template_1.setItem(testItemc, 8, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        test_reset = os.popen('adb shell /home/flex/bin/fct.sh button').read()
        ...
        testItemc += 1
        self.template_1.setItem(testItemc, 8, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        test_led_check = os.popen('adb shell /home/flex/bin/fct.sh led_check_error').read()
        ...
        testItemc += 1
        self.template_1.setItem(testItemc, 8, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        test_mute = os.popen('adb shell /home/flex/bin/fct.sh FCT.2.3').read()
        ...
        testItemc += 1
        self.template_1.setItem(testItemc, 8, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        test_muted = os.popen('adb shell /home/flex/bin/fct.sh FCT.2.3').read()
        ...
        self.template_1.setItem(testItemc, 9, QTableWidgetItem(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        #     ***************************unmount测试***************************
        chkPartHome = os.popen('adb shell /home/flex/bin/fct.sh umount_factory').read()
        self.RTestInfo.setText(
            "\n" + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + "\numount_factory:{0} ".format(chkPartHome))
        if (not chkPartHome.upper().__contains__("PASS")):
            self.RTestInfo.setText(
                "\n" + datetime.datetime.now().strftime("%Y%m%d%H%M%S") + "\numount_factory failed:{0} ".format(
                    chkPartHome))
            self.RTestInfo.setStyleSheet("QTextEdit#RTestInfo{background-color: red}")
            return
        self.RTestInfo.setStyleSheet("QTextEdit#RTestInfo{background-color: green}")
